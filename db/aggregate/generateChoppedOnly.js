// script to re-generate CSV of subsection of 10M records of data
...
let getRows = (entries, currRowInd, n, rows) => {
  if (n < choppedStart) {
    while (true) {
      ...
      if (target === Number(currRow.substring(0, splitInd))) {
        // target iterator === curr iterator
        currRowInd++;
      } else {
        break;
      }
...
fs.readFileAsync(readFile, "UTF-8").then(csv => writeFiles(csv.split("\r\n")));
